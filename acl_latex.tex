...
Our approach is to design the parallel corpus itself as a grammatical curriculum. We construct a five-module dataset that systematically covers present tense, negation, past tense, future tense, and question formation in French, with each module linked to the previous through \emph{minimal pairs}---sentence pairs that differ in exactly one grammatical dimension. A native Adja speaker then translates the complete set, producing a structured parallel corpus of approximately 4,000 sentence pairs. Crucially, the curriculum is embedded in the data, not in the training procedure: the resulting corpus can be used with any NMT architecture, any training schedule, and any optimization strategy.

The empirical results are striking. When combined with 10,000 randomly sourced parallel sentences from Tatoeba, our 4,000 structured sentences yield a BLEU score of approximately 20 on French to Adja translation. The same 10,000 random sentences alone produce a BLEU score of 2--3. This represents an order-of-magnitude improvement from data that required less than half the total translation effort. Module-level ablations confirm that specific grammatical components---not merely data cleanliness---drive the improvement.
...
\begin{enumerate}
    \item We introduce the first framework for designing parallel corpora as structured grammatical curricula, addressing a gap in the low-resource NMT literature where prior work has focused exclusively on filtering existing data or reordering it during training.

    \item We provide empirical evidence that corpus composition dominates corpus quantity for extremely low-resource NMT, with ablation studies isolating the contribution of individual grammatical modules.

    \item We release a reproducible, language-agnostic pipeline for linguistically informed corpus construction, designed for integration into language documentation and fieldwork workflows.
\end{enumerate}
...
M3 & Past tense            & $+$ tense shift   & GPT-4 API     & 813 \\
...
All French sentences were translated into Adja by a single native speaker of Adja who is also fluent in French. Adja (ISO 639-3: \texttt{ajg}) is a Gbe language spoken by approximately one million people in Benin and Togo. It has no Wikipedia, no significant web presence, and no pre-existing parallel corpus. The translator worked module by module, preserving the minimal-pair correspondence: the Adja translation of a negated sentence differs from its affirmative counterpart only in the grammatical features relevant to negation in Adja.
...
We use an 80/10/10 train/validation/test split with a \emph{group-aware} protocol: all transformations of a single Module~1 base sentence (its negation, past, future, and question forms) are assigned to the same split. This prevents data leakage from minimal pairs appearing across splits. The test set comprises 1,455 sentences shared across all experimental conditions.


\section{Experimental Setup}
\label{sec:experiments}

We design two main experiments, three smart-selection baselines, and a comprehensive set of ablation studies. All experiments use the same held-out test set of 1,455 sentences, the same model architecture, and the same training configuration (Table~\ref{tab:hyperparams}). Each condition is run with 3--5 random seeds to quantify variance.
...
We report SacreBLEU CITE[Post, 2018], chrF, chrF++ CITE[Popovi\'{c}, 2015], and TER on the shared test set. We report means and standard deviations across seeds for all conditions. Across all experiments, baselines, and ablations, we conducted a total of 224 training runs.
...
\midrule
200    & $9.4 \pm 0.7$  & $23.0 \pm 0.3$ & $0.5 \pm 0.1$ & $13.0 \pm 0.4$ \\
500    & $15.1 \pm 1.3$ & $26.6 \pm 0.7$ & $1.7 \pm 0.2$ & $18.7 \pm 0.3$ \\
1K     & $18.8 \pm 2.3$ & $28.8 \pm 0.4$ & $2.6 \pm 0.2$ & $22.1 \pm 0.2$ \\
2K     & $19.9 \pm 2.7$ & $29.2 \pm 0.7$ & $3.0 \pm 0.2$ & $25.7 \pm 0.7$ \\
4K     & $19.5 \pm 1.3$ & $29.1 \pm 0.5$ & $3.7 \pm 0.5$ & $28.8 \pm 0.6$ \\
10K    & ---            & ---            & $4.1 \pm 0.1$ & $30.8 \pm 0.4$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Condition} & \textbf{Train} & \textbf{BLEU}$\uparrow$ & $\Delta$ & \textbf{chrF}$\uparrow$ \\
\midrule
\textsc{Full}          & 3,823 & $\mathbf{22.9 \pm 0.3}$ & ---    & $30.1 \pm 0.3$ \\
\textsc{No-Past}       & 3,010 & $20.1 \pm 0.1$          & $-2.8$ & $28.3 \pm 0.2$ \\
\textsc{No-Negation}   & 3,024 & $19.4 \pm 0.5$          & $-3.5$ & $28.4 \pm 0.2$ \\
\textsc{No-Questions}  & 3,240 & $19.2 \pm 0.1$          & $-3.8$ & $28.2 \pm 0.2$ \\
\textsc{No-Future}     & 3,009 & $17.3 \pm 1.2$          & $-5.6$ & $27.5 \pm 0.4$ \\
\textsc{Base-Only}     &   815 & $8.0 \pm 0.4$           & $-14.9$ & $22.9 \pm 0.0$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\multicolumn{4}{l}{\textit{Minimal-pair structure (same sentences)}} \\
\textsc{Pairs-Intact}  & 3,823 & $\mathbf{22.9 \pm 0.3}$ & $\mathbf{30.1 \pm 0.3}$ \\
\textsc{Pairs-Broken}  & 3,823 & $5.4 \pm 0.7$           & $14.0 \pm 0.5$ \\
\midrule
\multicolumn{4}{l}{\textit{Verb lexical diversity}} \\
1 verb   &   216 & $2.8 \pm 0.5$  & $13.3 \pm 0.0$ \\
3 verbs  & ${\sim}$600 & $5.2 \pm 1.1$  & $14.4 \pm 1.1$ \\
5 verbs  & ${\sim}$950 & $8.1 \pm 0.5$  & $17.0 \pm 0.6$ \\
10 verbs & 3,823 & $22.9 \pm 0.3$ & $30.1 \pm 0.3$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{llrr}
\toprule
\textbf{Condition} & \textbf{Pronouns} & \textbf{BLEU}$\uparrow$ & \textbf{chrF}$\uparrow$ \\
\midrule
\textsc{All-8}      & je, tu, il, elle, nous, vous, ils, elles & $\mathbf{22.9 \pm 0.3}$ & $\mathbf{30.1 \pm 0.3}$ \\
\textsc{Reduced-4}  & je, tu, il, nous         & $19.5 \pm 5.2$ & $29.1 \pm 1.4$ \\
\textsc{Singular-3} & je, tu, il               & $18.6 \pm 2.4$ & $28.9 \pm 0.4$ \\
\textsc{Minimal-1}  & je                       & $15.9 \pm 2.3$ & $27.1 \pm 0.5$ \\
\bottomrule
\end{tabular}
\end{table}
...
Several extensions follow naturally from this work. First, replication on typologically diverse language pairs---particularly SOV languages, agglutinative morphology, and language pairs with greater structural divergence---is needed to establish the generalizability of the framework. Second, expanding the curriculum beyond five modules to include relative clauses, conditionals, passive voice, and complex subordination would test whether additional grammatical coverage yields continued gains or reaches diminishing returns. Third, combining structured data with cross-lingual transfer from related Gbe languages (Fon, Ewe) is a natural next step, given the availability of French-Fon (${\sim}$53K pairs) and French-Ewe (${\sim}$23K pairs) corpora. Finally, human evaluation by Adja speakers is essential to validate whether the automatic metric improvements correspond to genuine translation quality gains, particularly given the known limitations of BLEU for African languages CITE[Wang et al., 2024].

\section*{Limitations}

\paragraph{Single language pair.} All 224 experiments are on French$\rightarrow$Adja. Both languages are SVO and largely analytic, and the structured curriculum may exploit this typological alignment. We cannot claim the approach generalizes across typologies---particularly to SOV targets, agglutinative morphology, or languages with complex tonal interactions. Replication on diverse language pairs is needed, and we release all generation scripts and the five-module framework to facilitate this.
...
\paragraph{No human evaluation.} All conclusions rely on automatic metrics. BLEU correlates weakly with human judgments for African languages CITE[Wang et al., 2024], and COMET cannot be applied to Adja (which is outside COMET's training data). Human evaluation by Adja speakers is planned as follow-up work.
...
Table~\ref{tab:robustness} shows that the pattern is consistent across all completed models:
combining structured and random data substantially outperforms random-only data of equal or
greater size, confirming that the advantage is not architecture-specific.
