...
We report SacreBLEU CITE[Post, 2018], chrF, chrF++ CITE[Popovi\'{c}, 2015], ROUGE-L CITE[Lin, 2004], and perplexity on the shared test set. We report means and standard deviations across seeds for all conditions. Across all experiments, baselines, and ablations, we conducted a total of 280 training runs.
...
Table~\ref{tab:baselines} compares our structured corpus against three smart-selection baselines, each selecting 2K sentences from the 10K Tatoeba pool. All three baselines score BLEU 2.7--3.2, barely above random selection (BLEU 4.1) and far below \textsc{Structured-2K} (BLEU 19.9). Smart selection from unstructured data does not help: the selection strategy is irrelevant when the underlying data lacks grammatical structure.

\begin{table*}[t]
\centering
\small
\caption{Smart-selection baselines and structured+baseline combinations. Mean $\pm$ std over 5 seeds.}
\label{tab:baselines}
\begin{tabular}{lrrrr}
\toprule
\textbf{Condition} & \textbf{Train} & \textbf{BLEU}$\uparrow$ & \textbf{chrF}$\uparrow$ & \textbf{chrF++}$\uparrow$ \\
\midrule
\multicolumn{5}{l}{\textit{Standalone baselines (selected from 10K Tatoeba)}} \\
\textsc{Rand-10K}         & 7,200 & $4.1 \pm 0.1$  & $30.8 \pm 0.3$ & $27.6 \pm 0.2$ \\
\textsc{TF-IDF-Diverse}   & 1,614 & $2.8 \pm 0.3$  & $25.2 \pm 0.2$ & $22.6 \pm 0.2$ \\
\textsc{Length-Strat.}     & 1,608 & $3.2 \pm 0.3$  & $25.5 \pm 0.5$ & $22.9 \pm 0.4$ \\
\textsc{Vocab-Max.}       & 1,614 & $2.7 \pm 0.4$  & $25.5 \pm 0.4$ & $23.0 \pm 0.3$ \\
\textsc{Struct-2K}         & 1,800 & $19.9 \pm 2.7$ & $29.2 \pm 0.7$ & $28.9 \pm 0.8$ \\
\midrule
\multicolumn{5}{l}{\textit{Structured 4K + smart-selection baseline combinations}} \\
\textsc{S4K+TF-IDF-2K}    & 4,568 & $20.6 \pm 1.4$ & $35.8 \pm 1.1$ & $34.6 \pm 0.9$ \\
\textsc{S4K+Length-2K}     & 4,735 & $33.6 \pm 5.4$ & $46.6 \pm 3.7$ & $45.4 \pm 3.7$ \\
\textsc{S4K+Vocab-2K}     & 4,735 & $35.1 \pm 2.5$ & $48.2 \pm 1.7$ & $46.9 \pm 1.7$ \\
\textsc{S4K+All-Baselines} & 7,073 & $\mathbf{41.1 \pm 10.0}$ & $\mathbf{55.6 \pm 7.1}$ & $\mathbf{54.1 \pm 7.2}$ \\
\bottomrule
\end{tabular}
\end{table*}

The standalone baselines reveal a striking pattern: all three smart-selection strategies (BLEU 2.7--3.2) perform \emph{worse} than uniform random sampling (BLEU 4.1). Selecting 2K sentences from an existing 10K pool---regardless of the selection criterion---yields no advantage over random sampling, and falls far short of \textsc{Structured-2K} (BLEU 19.9). The smart-selection baselines cherry-pick sentences from an \emph{already-translated} 10K corpus---they answer ``given 10K translated sentences, which 2K should you keep?'' Our structured approach answers a fundamentally different question: ``given no existing translations, what should you create?'' When structured data is combined with smart-selection baselines (Table~\ref{tab:baselines}, bottom), performance improves dramatically: \textsc{S4K+All-Baselines} achieves BLEU 41.1, showing that the two data types are highly complementary.
...
\paragraph{Every module contributes.} Table~\ref{tab:module_loo} and \begin{figure}[t]
  ...
  \caption{Module leave-one-out ablation. Removing future tense (M4) causes the largest drop ($-5.1$ BLEU). Base-only (M1 present tense alone) collapses to BLEU~7.7, confirming that grammatical transformations provide essential learning signal.}
  \label{fig:module_ablation}
\end{figure} present the module leave-one-out results. Removing any single module degrades performance, with future tense (M4) causing the largest drop ($-5.1$ BLEU), followed by questions ($-3.7$), past tense ($-3.4$), and negation ($-3.2$). The \textsc{Base-Only} condition (M1 present-tense sentences alone) collapses to BLEU 7.7, confirming that the grammatical transformations in M2--M5 provide essential learning signal beyond what simple SVO sentences offer.
...
\midrule
\textsc{Full}          & 3,410 & $\mathbf{21.6 \pm 0.5}$ & ---      & $\mathbf{29.2 \pm 0.1}$ & $\mathbf{28.8 \pm 0.0}$ \\
\textsc{No-Past}       & 2,688 & $18.2 \pm 1.4$          & $-3.4$  & $27.9 \pm 0.1$          & $27.4 \pm 0.1$ \\
\textsc{No-Negation}   & 2,713 & $18.4 \pm 1.7$          & $-3.2$  & $27.8 \pm 0.4$          & $27.1 \pm 0.4$ \\
\textsc{No-Questions}  & 2,895 & $17.9 \pm 0.5$          & $-3.7$  & $27.7 \pm 0.1$          & $26.4 \pm 0.2$ \\
\textsc{No-Future}     & 2,692 & $16.5 \pm 1.6$          & $-5.1$  & $27.5 \pm 0.1$          & $26.9 \pm 0.2$ \\
\textsc{Base-Only}     &   760 & $7.7 \pm 0.5$           & $-13.9$ & $22.6 \pm 0.3$          & $20.8 \pm 0.3$ \\
\bottomrule
\end{tabular}
\end{table*}

The size-controlled ablation (Table~\ref{tab:module_size_ctrl}) confirms that this effect is not an artifact of data volume: with all conditions fixed at ${\sim}$1,000 sentences, distributing across all five modules (\textsc{Full-1K}, BLEU 19.8) vastly outperforms concentrating the same budget in M1 alone (\textsc{Base-1K}, BLEU 7.3). Module diversity matters independently of total data volume.

\begin{table*}[t]
\centering
\small
\caption{Size-controlled module ablation: all conditions at ${\sim}$1K sentences. Mean $\pm$ std over 3 seeds.}
\label{tab:module_size_ctrl}
\begin{tabular}{lrrr}
\toprule
\textbf{Condition} & \textbf{BLEU}$\uparrow$ & \textbf{chrF}$\uparrow$ & \textbf{chrF++}$\uparrow$ \\
\midrule
\textsc{Full-1K}       & $\mathbf{19.8 \pm 0.8}$ & $\mathbf{28.9 \pm 0.4}$ & $\mathbf{28.4 \pm 0.5}$ \\
\textsc{No-Past-1K}    & $16.6 \pm 2.0$          & $27.0 \pm 0.7$ & $26.5 \pm 0.6$ \\
\textsc{No-Quest.-1K}  & $16.0 \pm 0.4$          & $27.3 \pm 0.1$ & $26.0 \pm 0.1$ \\
\textsc{No-Neg.-1K}    & $15.5 \pm 3.2$          & $27.2 \pm 0.7$ & $26.6 \pm 0.8$ \\
\textsc{No-Fut.-1K}    & $16.5 \pm 1.2$          & $27.2 \pm 0.3$ & $26.5 \pm 0.4$ \\
\textsc{Base-1K}       & $7.3 \pm 0.5$           & $22.1 \pm 0.6$ & $20.2 \pm 0.6$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\paragraph{Minimal-pair structure is the key mechanism.} The most striking result is the minimal-pair ablation (Table~\ref{tab:structure_matters}, top; \begin{figure}[t]
  ...
  \caption{Evidence that \emph{structure}, not just data, drives quality. \textbf{Left:} Shuffling the minimal-pair alignment while keeping the exact same sentences reduces BLEU from 21.6 to 5.0. \textbf{Right:} BLEU increases steeply with verb lexical diversity (1 verb: 3.1; 10 verbs: 21.6).}
  \label{fig:structure_matters}
\end{figure}). \textsc{Pairs-Broken} uses the \emph{exact same sentences} as \textsc{Pairs-Intact}---the only difference is that the M1$\leftrightarrow$M2--M5 alignment has been randomly shuffled, breaking the contrastive pairing. BLEU drops from 21.6 to 5.0, a catastrophic decline of 16.6 points. This demonstrates that the model learns from the contrastive structure of minimal pairs, not merely from the surface forms of the sentences. The broken pairs create a contradictory training signal in which grammatical transformations no longer correspond to their base forms, yielding performance \emph{worse} than random data (BLEU 4.1).

\paragraph{Verb diversity drives generalization.} Table~\ref{tab:structure_matters} (bottom) and Figure~\ref{fig:structure_matters} (right) show that lexical diversity within the structured corpus is critical. With only 1 verb, the full module structure yields just BLEU 3.1; at 3 verbs, BLEU rises to 4.5; at 5 verbs, to 7.7; and at 10 verbs, to 21.6. The model requires sufficient lexical variety to learn generalizable grammatical mappings rather than memorizing verb-specific patterns.
...
\textsc{Pairs-Intact}  & 3,410 & $\mathbf{21.6 \pm 0.5}$ & $\mathbf{29.2 \pm 0.1}$ & $\mathbf{28.8 \pm 0.0}$ \\
\textsc{Pairs-Broken}  & 3,397 & $5.0 \pm 0.2$           & $13.3 \pm 0.4$          & $12.6 \pm 0.5$ \\
\midrule
\multicolumn{5}{l}{\textit{Verb lexical diversity}} \\
1 verb   &          190 & $3.1 \pm 0.1$ & $14.4 \pm 0.1$ & $13.4 \pm 0.1$ \\
3 verbs  & ${\sim}$530  & $4.5 \pm 0.7$ & $15.0 \pm 0.8$ & $14.4 \pm 0.6$ \\
5 verbs  & ${\sim}$830  & $7.7 \pm 1.2$ & $17.3 \pm 0.9$ & $17.0 \pm 1.1$ \\
10 verbs &        3,410 & $\mathbf{21.6 \pm 0.5}$ & $\mathbf{29.2 \pm 0.1}$ & $\mathbf{28.8 \pm 0.0}$ \\
\bottomrule
\end{tabular}
\end{table*}

\paragraph{Pronoun coverage shows diminishing returns.} Table~\ref{tab:pronoun} presents the pronoun coverage ablation. With only a single pronoun (\emph{je}), the model achieves BLEU 6.5. Adding more pronouns helps substantially (\textsc{Singular-3}: BLEU 20.0; \textsc{All-8}: BLEU 21.6), with diminishing returns beyond four pronouns. The jump from 1 to 3 pronouns (+13.5 BLEU) is far larger than from 4 to 8 (+0.8 BLEU), suggesting that a minimal paradigmatic contrast is essential but full coverage is not.
...
\midrule
\textsc{All-8}      & je, tu, il, elle, nous, vous, ils, elles & $\mathbf{21.6 \pm 0.5}$ & $\mathbf{29.2 \pm 0.1}$ & $\mathbf{28.8 \pm 0.0}$ \\
\textsc{Reduced-4}  & je, tu, il, nous         & $20.8 \pm 0.2$ & $29.4 \pm 0.1$ & $29.1 \pm 0.2$ \\
\textsc{Singular-3} & je, tu, il               & $20.0 \pm 0.2$ & $28.9 \pm 0.4$ & $28.2 \pm 0.4$ \\
\textsc{Minimal-1}  & je                       & $6.5 \pm 1.8$  & $20.9 \pm 1.3$ & $20.1 \pm 1.3$ \\
\bottomrule
\end{tabular}
\end{table*}
...
The structured approach and smart-selection baselines answer fundamentally different questions. All three baselines (BLEU 2.7--3.2) fall far short of \textsc{Structured-2K} (BLEU 19.9), confirming that no amount of clever selection from unstructured data can substitute for grammatically designed training data. However, combining structured and baseline data is highly effective: \textsc{S4K+Vocab-2K} reaches BLEU 35.1, showing that the two data types are complementary. For languages with no pre-existing parallel corpus, the structured approach creates new data from a grammatical template and a native speaker; where translated data already exists, combining the two strategies yields the best results.

\subsection{The chrF Gap}

Structured-only models exhibit a notable divergence between BLEU and chrF: \textsc{Structured-2K} achieves BLEU 19.9 but chrF only 29.2. This likely reflects the structured corpus's limited vocabulary (20 verbs, ${\sim}$5 objects each), which produces grammatically patterned but lexically narrow translations. Smart-selection baselines show even lower chrF (25.2--25.5), confirming that unstructured Tatoeba subsets provide neither grammatical scaffolding nor broad character-level coverage. Importantly, when structured data is combined with random data, chrF improves substantially (\textsc{R10K+S4K}: chrF 41.2), confirming that the two data types provide complementary signal---structured data contributes grammatical scaffolding, random data contributes lexical breadth.

\subsection{Module Interaction Effects}

The leave-one-out and additive ablations reveal complementary patterns. In the LOO analysis, removing future tense (M4) causes the largest single-module drop ($-5.1$ BLEU), exceeding questions ($-3.7$), past tense ($-3.4$), and negation ($-3.2$). However, the additive ablation (Figure~\ref{fig:additive_modules}) shows a different ordering: adding questions (M5) to the M1--M4 set provides the largest incremental gain ($+7.1$ BLEU), while adding future tense (M4) contributes $+3.3$. This asymmetry indicates that module contributions are order-dependent and non-additive. Explaining why requires targeted linguistic analysis of the Adja translations, which we leave to future work.
...
\paragraph{No human evaluation.} All conclusions rely on automatic metrics. BLEU correlates weakly with human judgments for African languages CITE[Wang et al., 2024]. Neural metrics (COMET, BERTScore) cannot be applied to Adja, which lies outside the training data of these models (see §\ref{sec:experiments}). Human evaluation by Adja speakers is planned as follow-up work.

\paragraph{Data quality confound.} Structured sentences are inherently cleaner and more predictable than Tatoeba sentences. The \textsc{Pairs-Broken} ablation---which uses identical sentences with shuffled alignment and shows a 16.6-point BLEU collapse---provides the strongest evidence that structure, not merely cleanliness, drives the improvement. However, we cannot fully disentangle the two factors.
...
across all models: combining structured and random data substantially outperforms
...
5-seed mean)     & R6K+S4K          & 21.4 & 38.6 \\
                 & R10K+S4K         & 22.4 & 39.2 \\
\midrule
NLLB-1.3B        & Random-10K       & 4.1$\pm$0.3  & 27.9$\pm$0.2 \\
(5-seed mean)    & Structured-4K    & 19.7$\pm$3.5 & 27.6$\pm$0.8 \\
                 & R6K+S4K          & 23.7$\pm$0.5 & 40.0$\pm$0.3 \\
                 & R10K+S4K         & 23.2$\pm$1.0 & 40.1$\pm$0.6 \\
\midrule
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{fig9_additive_modules.pdf}
  \caption{Additive module ablation: BLEU as modules are added incrementally from M1 (present tense base) through M5 (questions). Each module's incremental contribution is shown inside the bar. Questions (M5) provide the largest single gain (+7.1 BLEU). Mean $\pm$ std over 3 seeds.}
  \label{fig:additive_modules}
\end{figure}

\begin{table*}[h]
\centering
\small
\caption{Additive module ablation: modules added incrementally from M1 (present tense) through M5 (questions). Mean $\pm$ std over 3 seeds.}
\label{tab:additive_ablation}
\begin{tabular}{llrrrr}
\toprule
\textbf{Condition} & \textbf{Modules} & \textbf{Train} & \textbf{BLEU}$\uparrow$ & \textbf{chrF}$\uparrow$ & \textbf{chrF++}$\uparrow$ \\
\midrule
\textsc{Base-Only}  & M1           &   760 & $7.7 \pm 0.5$           & $22.6 \pm 0.3$ & $20.8 \pm 0.3$ \\
\textsc{M1+M2}      & M1--M2       & 1,292 & $10.3 \pm 0.2$          & $23.3 \pm 0.3$ & $21.6 \pm 0.2$ \\
\textsc{M1+M2+M3}   & M1--M3       & 1,944 & $11.2 \pm 2.8$          & $24.1 \pm 1.5$ & $22.5 \pm 1.3$ \\
\textsc{M1+M2+M3+M4}& M1--M4       & 2,597 & $14.5 \pm 3.4$          & $27.0 \pm 0.9$ & $25.7 \pm 1.0$ \\
\textsc{Full}        & M1--M5       & 3,410 & $\mathbf{21.6 \pm 0.5}$ & $\mathbf{29.2 \pm 0.1}$ & $\mathbf{28.8 \pm 0.0}$ \\
\bottomrule
\end{tabular}
\end{table*}
