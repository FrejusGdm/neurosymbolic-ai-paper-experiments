...
\textsc{Length-Strat.}     & 1,608 & TBD             & TBD             & TBD \\ %% TODO: rerun with decontaminated data
\textsc{Vocab-Max.}       & 1,614 & TBD             & TBD             & TBD \\ %% TODO: rerun with decontaminated data
...
The standalone baselines reveal a striking pattern: TF-IDF selection (BLEU 2.8) performs \emph{worse} than uniform random sampling (BLEU 4.1), while vocabulary-maximized selection achieves higher scores. This suggests that TF-IDF's emphasis on term frequency does not transfer useful signal for morphologically rich low-resource translation. The smart-selection baselines cherry-pick sentences from an \emph{already-translated} 10K corpus---they answer ``given 10K translated sentences, which 2K should you keep?'' Our structured approach answers a fundamentally different question: ``given no existing translations, what should you create?'' When structured data is combined with smart-selection baselines (Table~\ref{tab:baselines}, bottom), performance improves dramatically: \textsc{S4K+All-Baselines} achieves BLEU 41.1, showing that the two data types are highly complementary.
