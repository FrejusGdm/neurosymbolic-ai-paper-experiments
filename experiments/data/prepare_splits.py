...
def load_parallel_csv(path, src_col="french", tgt_col="adja_translation"):
    ...
    if src_col not in df.columns:
        raise ValueError(f"Source column '{src_col}' not found in {path}. Columns: {list(df.columns)}")
    if tgt_col not in df.columns:
        raise ValueError(f"Target column '{tgt_col}' not found in {path}. Columns: {list(df.columns)}")
