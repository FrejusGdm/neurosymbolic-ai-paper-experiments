...
Downloads data from HF Hub, fine-tunes NLLB-200 with a custom training loop
(Adafactor + constant-with-warmup), evaluates on the shared test set, and
uploads test_metrics.json to a private HF results repo.

Uses a custom `aj_Latn` language token initialized from Ewe embeddings,
and text preprocessing matching NLLB's pretraining pipeline.
...
SRC_LANG = os.environ.get("SRC_LANG", "fra_Latn")
TGT_LANG = os.environ.get("TGT_LANG", "aj_Latn")
SIMILAR_LANG = os.environ.get("SIMILAR_LANG", "ewe_Latn")
...
def init_lang_token(model, tokenizer, new_lang="aj_Latn", similar_lang="ewe_Latn"):
    """Register aj_Latn and copy Ewe embeddings to initialize it."""
